...
def init_vector_db():
    sync_from_s3()
    emb = GoogleGenerativeAIEmbeddings(model="models/gemini-embedding-2-preview", model_api_version="v1")
    v_db = Chroma(persist_directory=LOCAL_DB_PATH, embedding_function=emb, collection_name="risk_policies")
    return v_db
...
if prompt := st.chat_input("Ask about high risks, transaction defaults, or company policies..."):
    st.session_state.history.append({"role": "user", "content": prompt})
    with st.chat_message("user"): st.write(prompt)
    
    with st.spinner("🤖 Consulting Specialist Agents..."):
        try:
            # Running the graph and extracting the clean content
            result = agent_brain.invoke({"messages": [HumanMessage(content=prompt)]})
            
            # Extract the last message object from the graph output
            ans = result["messages"][-1]
            
            # Format the display: Agent Name + Clean Text Content
            agent_name = ans.name.replace("_", " ") if hasattr(ans, 'name') else "Assistant"
            clean_content = ans.content # This ensures we don't get the "signature" blocks
